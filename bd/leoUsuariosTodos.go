...
func LeoUsuariosTodos(ID string, page int64, search string, tipo string) ([]*models.Usuario, bool) {
	...
	query2 := bson.M{
		"apellidos": bson.M{"$regex": `(?i)` + search},
	}
	//Ahora ejecutamos el find que devuelve el resultado en un cursor
	cur2, err := col.Find(ctx, query2, findOptions)
	if err != nil {
		// si no fue satisfactoria la búsqueda en la BD
		fmt.Println(err.Error())
		//Devuelve results vacio y false
	...
	//Busco el registro por apellido
	// El next me permite avanzar a los registros siguientes
	for cur2.Next(ctx) {

		//defino un usuario para trabajar con el elemento que puedo después incluir en results
		var s models.Usuario

		//grabo lo que trae el cursor en el modelo de usuario para poder acceder a los campos
		err := cur2.Decode(&s)
		if err != nil {
			fmt.Println(err.Error())
			return results, false
		...
		//creo una variable para consultar sobre la relacion con el usuario
		var r models.Relacion
		r.UsuarioID = ID
		r.UsuarioRelacionID = s.ID.Hex()
		...
		//evaluo si el tipo es follow sólo listo los que yo estoy siguiendo
		if tipo == "follow" && encontrado == true {
			incluir = true
		}
		if r.UsuarioRelacionID == ID {
			//en caso de estar intentando verificar que me siga a mi mismo
			incluir = false
		}
		if incluir == true {
			//hago un blanqueo de los campos que no me interesa incluir
